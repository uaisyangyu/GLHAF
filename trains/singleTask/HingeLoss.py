...
class HingeLoss(nn.Module):
    ...
    def forward(self, ids, feats, margin=0.1):
        B, F = feats.shape
        #是s,t是重复feats得到的矩阵，表示所有样本对的特征组合，id是对应类别的标签矩阵，表示所有样本对的类别组合
        ...
        #计算所有样本对的余弦相似度，结果是一个形状为(B ** 2)的一维张量。
        cosine = self.compute_cosine(s, t) # B**2
        # 使用对角线掩码equal_mask去除对角线上的元素（即自身与自身的比较）。
        # s_ids、t_ids和cosine都被重新调整为形状(B, B - 1)，表示每个样本与其他样本的比较。
        ...
        # sim_mask是一个布尔矩阵，表示哪些样本对属于同一类别。
        # margin是动态计算的间隔值，根据样本对的类别差异调整。
        sim_mask = (s_ids == t_ids) # B X (B-1)
        margin = 0.15 * abs(s_ids - t_ids)#[~sim_mask].view(B, B - 3)
        #遍历每个样本，分别计算其与正样本和负样本的相似度。使用 torch.max 计算 hinge loss，确保损失值非负。最终，将所有样本的损失值平均。
        loss = 0
        loss_num = 0
