...
class OTDBFusion(nn.Module):
    ...
    def ot_plan(self, X, Y):
        ...
        nu = torch.empty(B, Ly, 1, device=x.device).fill_(1.0 / Ly)

        # 计算 Kernel K = exp(-C / eps)
        K = torch.exp(-C / self.eps)

        u = torch.ones(B,L,1,device=x.device)/L  # 初始化
        v=torch.ones(B,Ly,1,device=x.device)/Ly
        ...
        Pi = u * K * v.transpose(1, 2)  # shape (B, L, Ly)
        return Pi,C
